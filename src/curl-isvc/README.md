...
```bash
curl-isvc [--port port] [project] <inference-service> <prompt>
```
...
# Use a different local port instead of 8080
...
3. Refuses to run if another process already listens on the selected local port; it never kills or replaces that listener.
4. Port-forwards the selected local port (default `8080`) to the predictor pod's model port (`8000`).
